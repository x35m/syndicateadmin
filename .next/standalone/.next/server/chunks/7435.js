"use strict";exports.id=7435,exports.ids=[7435],exports.modules={9487:(e,t,a)=>{a.d(t,{db:()=>s});let i=new(a(5900)).Pool({connectionString:process.env.DATABASE_URL||process.env.DATABASE_PUBLIC_URL,ssl:{rejectUnauthorized:!1}});class r{materialSelect(e){return`SELECT m.id,
                   m.title,
                   m.content,
      ...
      FROM materials
    `)).rows[0]}async close(){await i.end()}async addFeed(e,t,a){return(await i.query(`INSERT INTO feeds (url, title, description, status)
       VALUES ($1, $2, $3, 'active')
       ON CONFLICT (url) DO UPDATE SET
         title = COALESCE(EXCLUDED.title, feeds.title),
         description = COALESCE(EXCLUDED.description, feeds.description),
         status = 'active'
       RETURNING id, url, title, description, last_fetched as "lastFetched", status, created_at as "createdAt"`,[e,t,a])).rows[0]}async getAllFeeds(){return(await i.query(`SELECT id, url, title, description, last_fetched as "lastFetched", status, created_at as "createdAt"
       FROM feeds
       WHERE status <> 'deleted'
       ORDER BY created_at DESC`)).rows}async getActiveFeeds(){return(await i.query(`SELECT id, url, title, description, last_fetched as "lastFetched", status, created_at as "createdAt"
       FROM feeds
       WHERE status = 'active'
       ORDER BY created_at DESC`)).rows}async getFeedsByIds(e){return 0===e.length?[]:(await i.query(`SELECT id, url, title, description, last_fetched as "lastFetched", status, created_at as "createdAt"
       FROM feeds
       WHERE id = ANY($1::int[])
         AND status <> 'deleted'
       ORDER BY created_at DESC`,[e])).rows}async getFeedById(e){return(await i.query(`SELECT id, url, title, description, last_fetched as "lastFetched", status, created_at as "createdAt"
       FROM feeds
       WHERE id = $1`,[e])).rows[0]}async updateFeedFetchTime(e){await i.query("UPDATE feeds SET last_fetched = NOW() WHERE id = $1",[e])}async updateFeedTitle(e,t){await i.query("UPDATE feeds SET title = $1 WHERE id = $2",[t,e])}async updateFeedStatus(e,t){await i.query("UPDATE feeds SET status = $1 WHERE id = $2 AND status <> 'deleted'",[t,e])}async deleteFeed(e){await i.query("UPDATE feeds SET status = $1 WHERE id = $2",["deleted",e])}async getSetting(e){let t=await i.query("SELECT value FROM settings WHERE key = $1",[e]);return t.rows[0]?.value||null}async setSetting(e,t){await i.query(`INSERT INTO settings (key, value, updated_at) 
       VALUES ($1, $2, NOW())
       ON CONFLICT (key) 
       DO UPDATE SET value = $2, updated_at = NOW()`,[e,t])}async getSettings(){let e=await i.query("SELECT key, value FROM settings"),t={};return e.rows.forEach(e=>{t[e.key]=e.value}),t}async getAutomationConfig(){let e=await this.getSetting("automation_config");if(!e)return null;try{return JSON.parse(e)}catch(e){return console.warn("Failed to parse automation config from settings:",e),null}}async setAutomationConfig(e){await this.setSetting("automation_config",JSON.stringify(e))}async updateMaterialSummary(e,t){let a=[],r=[],s=1;void 0!==t.summary&&(a.push(`summary = $${s}`),r.push(t.summary),s++),void 0!==t.metaDescription&&(a.push(`meta_description = $${s}`),r.push(t.metaDescription),s++),void 0!==t.sentiment&&(a.push(`sentiment = $${s}`),r.push(t.sentiment),s++),void 0!==t.contentType&&(a.push(`content_type = $${s}`),r.push(t.contentType),s++),!0===t.setProcessed&&(a.push("processed = TRUE"),a.push(`status = CASE
